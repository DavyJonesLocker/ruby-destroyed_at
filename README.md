...
`where(destoyed_at: nil)`. It also defines a scope called `.destoyed`
which removes the default scope and finds the records of a relation
`where.not(destoyed_at: nil)`. This is different than calling `unscoped`
on the class/relation. Where unscoped will remove all scoping from a
relation, `.destoyed` only removes the default scope generated by the
`DestoyedAt` module, making it safe to call on relations.
